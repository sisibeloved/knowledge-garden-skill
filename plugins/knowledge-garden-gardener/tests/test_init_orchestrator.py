...
def test_init_completes_all_steps_when_oauth_done(tmp_path: Path):
    vault = tmp_path / "Garden"
    config_path = vault / "_Config" / "gardener.config.yaml"
    checkpoint = vault / ".garden-init.json"
    calls = []
    client = NotionClient("http://mcp", "x", "x", post=_ok_post(calls))

    # oauth_already_done=True 模拟用户已完成授权
    result = run_init(
...
def test_init_pauses_at_oauth_when_not_done(tmp_path: Path):
    vault = tmp_path / "Garden"
    checkpoint = vault / ".garden-init.json"
    client = NotionClient("http://mcp", "x", "x", post=_ok_post([]))
    ...
    # vault 骨架应已建好(OAuth 之前能做的都做了)
    assert (vault / "Concepts").exists()
    # 但 Notion 库还没建(checkpoint 记录待 OAuth)
...
def test_init_resumes_from_checkpoint(tmp_path: Path):
    ...
    # 第二次:OAuth 完成 → 续跑,完成 Notion 库 + config + 插件
...
def test_init_completed_state_short_circuits(tmp_path: Path):
    ...
    assert result == COMPLETED
    assert calls == []  # 没重复调 create_database
